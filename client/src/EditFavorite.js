...
import { fetchRover, fetchAPOD, BACK_SERVER_URL } from "./NasaAPIs";
...
export default class EditFavorite extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    let response = await axios.post(`${BACK_SERVER_URL}/account/favorites/${this.props.location.state.api}/fetch`, acc);
  ...
  toggleLike = async () => {
    await axios.delete(`${BACK_SERVER_URL}/account/favorites/${this.state.photo.api}/delete`, {
  ...
  handleCommentUpdate = async newComment => {
    ...
    axios.post(`${BACK_SERVER_URL}/account/favorites/${this.state.photo.api}/edit`, {
      _id: this.props.location.state.email,
      data: {
        _id: id,
        comment: newComment
